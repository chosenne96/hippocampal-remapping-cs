...
//     // fmri
//     public static float fmriTimer = 0f;
//     public static StreamWriter fmriWriter;
//     public static bool fmriready = false;
...
//         }
//         if (fmriready)
//             fmriTimer += Time.deltaTime;
//         if (Input.GetKeyDown(KeyCode.Alpha5) && fmriready)
//             fmriWriter.WriteLine("shot" + "\t" + RemapManager.fmriTimer.ToString()); 
//     }
//     void FixedUpdate() {
//     if (Input.GetKey(SbjManager.leftKey) && fmriready)
//         fmriWriter.WriteLine("left" + "\t" + RemapManager.fmriTimer.ToString());
//     if (Input.GetKey(SbjManager.selectKey) && fmriready)
//         fmriWriter.WriteLine("select" + "\t" + RemapManager.fmriTimer.ToString());
//     if (Input.GetKey(SbjManager.forwardKey) && fmriready)
//         fmriWriter.WriteLine("forward" + "\t" + RemapManager.fmriTimer.ToString());
//     if (Input.GetKey(SbjManager.rightKey) && fmriready)
//         fmriWriter.WriteLine("right" + "\t" + RemapManager.fmriTimer.ToString());
//     }
//     void SaveUserInput()
//     {
...
//         {
//             new Vector3(-7.8f, 0f, 7.2f),   // A1
//             new Vector3(-7.5f, 0f, -8.2f)   // A2
//         };

//         Vector3[] contextB = new Vector3[]
//         {
//             new Vector3(7.6f, 0f, -7.9f),     // B1
//             new Vector3(7.3f, 0f, 7.3f)     // B2
//         };
...
//         else if (sbj_task == 6 || sbj_task == 7 || sbj_task == 8)
...
//         showGUI = true;
//         guiMessage = "잠시 기다려주세요.";
//         yield return new WaitUntil(() => Input.GetKeyDown(KeyCode.Space));
//         showGUI = false;
//         showGUI = true;
//         guiMessage = "준비되셨으면 선택 버튼을 누르세요.";
//         yield return new WaitUntil(() => Input.GetKeyDown(KeyCode.Alpha4));
//         showGUI = false;
        
//         fmriWriter = File.AppendText("./output/sbj" + SbjManager.sbj_num + "_coin" + (SbjManager.curr_coin_sess + 1) + "_fmri.txt");
//         fmriready = true;
//         fmriWriter.WriteLine($"Subject ID: {SbjManager.sbj_num}");
//         fmriWriter.WriteLine("Date: " + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
//         showGUI = true;
//         guiMessage = "준비 시간입니다.";
//         yield return new WaitUntil(() => Input.GetKeyDown(KeyCode.Alpha5));
//         showGUI = false;

//         SbjManager.showFixation = true; fmriWriter.WriteLine($"fix_start" + "\t" + fmriTimer.ToString());
//         yield return new WaitForSeconds(4f);
//         SbjManager.showFixation = false; fmriWriter.WriteLine($"fix_end" + "\t" + fmriTimer.ToString());
        
//         SbjManager.curr_coin_run = 0;
        
//         showGUI = true; fmriWriter.WriteLine($"ins_start" + "\t" + fmriTimer.ToString());
//         // guiMessage = "You are a farmer.\n\nWalk around the gardens\nand collect seed bags.";
//         guiMessage = "당신은 농부입니다.\n\n여러 텃밭을 돌아다니며\n씨앗 주머니를 주워주세요.";
//         yield return new WaitForSeconds(6f);
//         showGUI = false; fmriWriter.WriteLine($"ins_end" + "\t" + fmriTimer.ToString());

//         if (SbjManager.curr_coin_sess == 0) {
//         SbjManager.coinWriter1.WriteLine($"scene order for run1: {string.Join(", ", SbjManager.sbj_coin_order[0])}");
//         SbjManager.coinWriter1.WriteLine($"scene order for run2: {string.Join(", ", SbjManager.sbj_coin_order[1])}");
//         SbjManager.coinWriter1.WriteLine($"scene order for run3: {string.Join(", ", SbjManager.sbj_coin_order[2])}");
//         SbjManager.coinWriter1.Flush();
//         }
//         else if (SbjManager.curr_coin_sess == 1) {
//         SbjManager.coinWriter2.WriteLine($"scene order for run1: {string.Join(", ", SbjManager.sbj_coin_order[0])}");
//         SbjManager.coinWriter2.WriteLine($"scene order for run2: {string.Join(", ", SbjManager.sbj_coin_order[1])}");
//         SbjManager.coinWriter2.WriteLine($"scene order for run3: {string.Join(", ", SbjManager.sbj_coin_order[2])}");
//         SbjManager.coinWriter2.Flush();
//         }
//         else if (SbjManager.curr_coin_sess == 2) {
//         SbjManager.coinWriter3.WriteLine($"scene order for run1: {string.Join(", ", SbjManager.sbj_coin_order[0])}");
//         SbjManager.coinWriter3.WriteLine($"scene order for run2: {string.Join(", ", SbjManager.sbj_coin_order[1])}");
//         SbjManager.coinWriter3.WriteLine($"scene order for run3: {string.Join(", ", SbjManager.sbj_coin_order[2])}");
...
//             fmriWriter.WriteLine($"fix_start" + "\t" + fmriTimer.ToString());
//             fmriWriter.Flush();
//             SbjManager.showFixation = true;
...
//             if (curr_idx == 4)
//             {
//                 fmriWriter.WriteLine($"scene_ended" + "\t" + fmriTimer.ToString());
//                 fmriWriter.Flush();
//                 yield return new WaitForSeconds(6f); 
//                 SbjManager.showFixation = false;
//                 // fmriWriter.WriteLine($"fix_end" + "\t" + fmriTimer.ToString());
//                 showGUI = true; 
//                 // fmriWriter.WriteLine($"ins_start" + "\t" + fmriTimer.ToString());
//                 // guiMessage = "You've collected enough seeds.";
//                 guiMessage = "이제 씨앗을 충분히 주웠습니다.";
//                 yield return new WaitForSeconds(6f); 
//                 showGUI = false; 
//                 // fmriWriter.WriteLine($"ins_end" + "\t" + fmriTimer.ToString());
//             }
//             if (curr_idx == 4 && SbjManager.curr_coin_sess == 2)
//             {
//             SbjManager.showFixation = false; fmriWriter.WriteLine($"fix_end" + "\t" + fmriTimer.ToString());
//             showGUI = true; fmriWriter.WriteLine($"ins_start" + "\t" + fmriTimer.ToString());
//             fmriWriter.Flush();
...
//     {   
//         isCompleted = false;  // 코루틴 시작 시 초기화
...
//         yield return new WaitUntil(() => Input.GetKeyDown(KeyCode.Alpha4));
//         showGUI = false;
//         fmriWriter = File.AppendText("./output/sbj" + SbjManager.sbj_num + "_remap1-1_fmri.txt");
//         fmriready = true;
...
//         showGUI = true; fmriWriter.WriteLine($"ins_start" + "\t" + fmriTimer.ToString());
//         guiMessage = "당신은 영양가가 가장 많고\n탐스러운 농작물을 수확하는 것이\n목적입니다.";
//         yield return new WaitForSeconds(6f);
//         showGUI = false; fmriWriter.WriteLine($"ins_end" + "\t" + fmriTimer.ToString());
//         showGUI = true; fmriWriter.WriteLine($"ins_start" + "\t" + fmriTimer.ToString());
//         guiMessage = "텃밭마다 영양분이 많은 위치와\n 훨씬 더 많은 위치가 있습니다."; fmriWriter.WriteLine($"ins_start" + "\t" + fmriTimer.ToString());
//         yield return new WaitForSeconds(6f);
//         showGUI = false; fmriWriter.WriteLine($"ins_end" + "\t" + fmriTimer.ToString());
...
//             SbjManager.showFixation = true; fmriWriter.WriteLine($"fix_start" + "\t" + fmriTimer.ToString());
//             AsyncOperation asyncLoad = SceneManager.LoadSceneAsync(sceneName);                
//             asyncLoad.allowSceneActivation = true;  // true로 변경
//             yield return new WaitUntil(() => isCompleted);
//             isCompleted = false;
...
//     yield return new WaitUntil(() => Input.GetKeyDown(KeyCode.Alpha4));
//     showGUI = false;
//     fmriWriter = File.AppendText("./output/sbj" + SbjManager.sbj_num + "_remap1-2_fmri.txt");
//     fmriready = true;
//     fmriWriter.WriteLine($"Subject ID: {SbjManager.sbj_num}");
//     fmriWriter.WriteLine("Date: " + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
//     showGUI = true;
...
//         // fmriWriter.WriteLine($"ins_start" + "\t" + fmriTimer.ToString());
//         // guiMessage = "You will now learn\nthe mushroom locations with feedback.";
...
//         {
//             if (curr_idx == 20)
//                 SbjManager.sbj_test_order[20] = SbjManager.sbj_test_order[0];
...
//             SbjManager.curr_scene = sceneName;
//             SbjManager.curr_trial = curr_idx;
...
//             {
//                 SbjManager.showFixation = false; fmriWriter.WriteLine($"fix_end" + "\t" + fmriTimer.ToString());
//                 SbjManager.showFixation = false; fmriWriter.WriteLine($"scene_end" + "\t" + fmriTimer.ToString());
//                 fmriWriter.Flush();
//             }
//         }
//     }
...
//     yield return new WaitUntil(() => Input.GetKeyDown(KeyCode.Alpha4));
//     showGUI = false;
//     fmriWriter = File.AppendText("./output/sbj" + SbjManager.sbj_num + "_remap2-1_fmri.txt");
//     fmriready = true;
//     fmriWriter.WriteLine($"Subject ID: {SbjManager.sbj_num}");
//     fmriWriter.WriteLine("Date: " + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
//     showGUI = true;
...
//     guiMessage = "텃밭에 영양분의 위치가 뒤바뀌었습니다.";
//     yield return new WaitForSeconds(6f);
//     showGUI = false; fmriWriter.WriteLine($"ins_end" + "\t" + fmriTimer.ToString());
...
//     SbjManager.coinWriter1.WriteLine($"Selected setComb indices: [{string.Join(", ", selectedSetComb)}]");
//     SbjManager.coinWriter1.WriteLine($"Selected seqtoenv index: {seqtoenv_idx}");
...
//         {
//             int chunkIdx = setComb[setCombIdx, i];
//             if (chunkIdx >= 0 && chunkIdx < playerstart.Count && playerstart[chunkIdx] != null)
//             {
...
//     SbjManager.pre_playerstart.Clear();
//     SbjManager.interm_playerstart.Clear();
//     SbjManager.post_playerstart.Clear();
...
//             else
//             {
//                 bool isNull = (sourceIdx >= 0 && sourceIdx < pre_playerpositions.Count) ? (pre_playerpositions[sourceIdx] == null) : true;
...
//     if (SbjManager.curr_coin_sess == 0)
//     {
//         Debug.Log($"curr_coin_env: {SbjManager.curr_coin_env}, curr_coin_run: {SbjManager.curr_coin_run}, pre_playerstart.Count: {SbjManager.pre_playerstart.Count}");
//         Debug.Log($"curr_coin_run: {SbjManager.curr_coin_run}, Vector3[] Length: {SbjManager.pre_playerstart[SbjManager.curr_coin_env]?.Length}");
